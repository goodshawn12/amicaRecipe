...
function runamica15_prep(dat,data_name,data_folder,param_name,param_folder,varargin)

if nargin < 1
    help runamica15_prep;
end

% define output directory - create a folder under the current working directory
outdir = [ pwd filesep 'param' filesep ];
...
% if isnumeric(dat)
%     full_filename = [data_folder,data_name,'.fdt'];
%     disp(['Writing data file: ' full_filename]);
%     floatwrite(dat, full_filename);
%     chans = size(dat,1);
%     frames = size(dat,2);
%     file = full_filename;
% end            
...
fid = fopen([param_folder, param_name, '.param'],'w');
if fid < 1
    errordlg(['cannot create file in ' param_folder],'Bad Input','modal');
    return;
end

%%%%%%%%%%% generate param file for Comet %%%%%%%%%%%%
file_comet = sprintf('/home/shhsu/data/%s',[data_name,'.fdt']);
file_outdir = sprintf('/home/shhsu/amica/amicaout/emotion/%s', param_name );
